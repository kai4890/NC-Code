...
# Section number followed by a short title — structural label, not policy evidence
# Matches: "3.2A.5 Remote Working Requirements", "3.5 Access control"
_SECTION_HEADING = re.compile(
    r'^\d+(\.\d+)*[A-Z]?\s+[A-Z][^.]{0,60}$'
)

# Obligation verbs — narrower set used for the short-clause check
_OBLIGATION_VERBS = re.compile(
    r'\b(?:must|shall|should|will|requires?|ensures?)\b',
    re.IGNORECASE,
)
...
def _is_low_quality(clause: str) -> bool:
    ...
    if not _VERBS.search(clause):
        return True
    # Section heading label (e.g. "3.2A.5 Remote Working Requirements")
    if _SECTION_HEADING.match(clause.strip()):
        return True
    # Short clause with no obligation verb — likely a title or label
    if len(words) < 15 and not _OBLIGATION_VERBS.search(clause):
        return True
    return False
