...
def get_neo_analysis(control_id, control_text, framework, status, score, matched_clause):
    client = get_neo_client()
    if client is None:
        return "Neo could not initialise — ANTHROPIC_API_KEY missing from secrets."

    system_prompt = """You are Neo, an expert AI compliance advisor specialising in Australian defence sector cybersecurity frameworks including ISO 27001, DISP, and ASD Essential Eight.

You give precise, actionable, plain-English recommendations to help organisations improve their compliance posture against Australian defence requirements.

Always respond in exactly this format — four numbered points, no preamble, no sign-off:

1. WHY THIS SCORE: Explain specifically why this control received this score based on the matched clause provided.
2. COMPLIANCE RISK: The specific risk this gap creates for an Australian defence supplier bidding on contracts.
3. REMEDIATION ACTION: One concrete, prioritised action the organisation should take immediately.
4. EVIDENCE REQUIRED: What a DISP assessor would need to see to mark this control as compliant.

Keep each point to 2 sentences maximum. Be direct and specific. Never give generic advice."""

    user_message = f"""Control ID: {control_id}
Framework: {framework}
Control requirement: {control_text}
Compliance status: {status} (confidence score: {score:.2f})
Best matching clause found in the policy document: "{matched_clause}"

Provide your four-point analysis."""
...
def _render_fw_tab(
    ...
    with col_table:
        ...
        sort_fw = st.checkbox("Sort by least compliant", key=f"sort_fw_{fw_name}")
        
        display_results = list(results)
        if sort_fw:
            display_results.sort(key=lambda r: r["confidence_score"])
            
        st.markdown(_controls_table(display_results), unsafe_allow_html=True)
...
def main() -> None:
    ...
    with tabs[0]:
        ...
        with chart_right:
            ...
            sort_breakdown = st.checkbox("Sort by least compliant", key="sort_breakdown")
            status_cfg = {
            ...
            for fw in active_fws:
                flat_results.extend(all_results[fw])
                
            if sort_breakdown:
                flat_results.sort(key=lambda r: r["confidence_score"])
